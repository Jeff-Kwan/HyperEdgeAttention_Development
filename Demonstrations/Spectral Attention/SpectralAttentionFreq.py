...
def softmax_diag_offdiag(tensor):
    """
    tensor: (B, C, H, W)
    Applies softmax separately on the diagonal and off-diagonal elements (over each matrix).
    """
    B, C, H, W = tensor.shape
    device = tensor.device

    # Create diagonal mask (H, W)
    diag_mask = torch.eye(H, W, dtype=torch.bool, device=device)  # (H, W)
    diag_mask = diag_mask.unsqueeze(0).unsqueeze(0)  # (1, 1, H, W)

    # Broadcast masks to (B, C, H, W)
    diag_mask = diag_mask.expand(B, C, -1, -1)
    offdiag_mask = ~diag_mask

    # Extract diagonal elements → (B, C, min(H, W))
    diag_elements = tensor.masked_select(diag_mask).view(B, C, min(H, W))

    # Extract off-diagonal elements → (B, C, H * W - min(H, W))
    offdiag_elements = tensor.masked_select(offdiag_mask).view(B, C, H * W - min(H, W))

    # Apply softmax over last dimension
    diag_softmax = torch.softmax(diag_elements, dim=-1)
    offdiag_softmax = torch.softmax(offdiag_elements, dim=-1)

    # Reconstruct the full tensor
    out = torch.zeros_like(tensor)

    # Assign processed values back
    out.masked_scatter_(diag_mask, diag_softmax.view(-1))
    out.masked_scatter_(offdiag_mask, offdiag_softmax.view(-1))
