...
class HyperEdgeAttention(nn.Module):
    ...
    def __init__(self, channels, edges, heads, bias=False):
        ...
        self.WE = nn.Sequential(nn.Linear(channels, channels, bias=True), nn.Softplus())
    ...
    def forward(self, x):
        ...
        edges = torch.randn(self.edges, B, C, device=x.device, requires_grad=False)
        ...
        with sdpa_kernel([SDPBackend.EFFICIENT_ATTENTION, SDPBackend.FLASH_ATTENTION, SDPBackend.CUDNN_ATTENTION]):
            ...
            WE = F.normalize(self.WE(z), p=1, dim=0) * self.edges
            y = self.MHA2(x, self.norm(z), z * WE, need_weights=False)[0]
