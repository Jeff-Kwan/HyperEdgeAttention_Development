...
class HyperEdgeAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        z = self.z_norm(weights.transpose(1, 2) @ x)
...
class ConvBlock(nn.Module):
    def __init__(self, in_c, h_c, out_c, bias=False, dropout=0.0):
        super(ConvBlock, self).__init__()
        assert h_c % 2 == 0, "h_c must be divisible by 2."
        self.in_norm = RMSNormTranspose(1, in_c)
        self.path1 = nn.Conv2d(in_c, h_c//2, 3, 1, 1, bias=bias)
        self.path2 = nn.Conv2d(in_c, h_c//2, 3, 1, 2, dilation=2, bias=bias)
    ...
    def forward(self, x):
        ...
        x = torch.cat([self.path1(x), self.path2(x)], dim=1)
        x = self.out_conv(x)
        return x
...
class CNNEmbedding(nn.Module):
    def __init__(self, in_c, out_c, dropout=0.0):
        ...
        self.convs = nn.ModuleList([
            ConvBlock(out_c//4, out_c//4, out_c//4, False, dropout),  # 2x2
            ConvBlock(out_c, out_c, out_c, False, dropout)            # 4x4
        ])
...
class PatchViT(nn.Module):
    def __init__(self, model_params):
        ...
        self.CNN_embed = CNNEmbedding(in_channels, channels[0], dropout=dropout)
        self.downs = nn.ModuleList([
            nn.Sequential(
                RMSNormTranspose(1, channels[i], elementwise_affine=False),
                nn.Conv2d(channels[i], channels[i+1], 2, 2, 0, bias=False))
...
if __name__ == "__main__":
    ...
    "convs": [128, 128, 128, 128],
    "attns": [[256, 6], [256, 8], 12, 12],
