...
class RandomViT(nn.Module):
    def __init__(self, model_params):
        super(RandomViT, self).__init__()
        ...
        self.out = nn.Sequential(nn.RMSNorm(n_embed*(num_layers+1), elementwise_affine=False),
                                   nn.Linear(n_embed*(num_layers+1), out_channels))
    ...
    def forward(self, x):
        ...
        for i in range(self.n_layers):
            random_query = torch.randn(self.n_vectors, x_shape[0], self.n_embed, device=x.device)
            # z = z + learnt_query
            z = self.latent_layers[i](x, random_query)
            x = self.img_layers[i](x, z, x_shape)
            z = z - random_query
            z_out.append(self.out_norms[i](torch.mean(z, dim=0)))

        random_query = torch.randn(self.n_vectors, x_shape[0], self.n_embed, device=x.device)
        z = self.latent_layers[-1](x, random_query)
        z = z - random_query
        z_out.append(self.out_norms[-1](torch.mean(z, dim=0)))
        z_out = torch.cat(z_out, dim=-1)
...
if __name__ == "__main__":
    ...
    # Instantiate RandomViT
    model = RandomViT(model_params).to(device)
