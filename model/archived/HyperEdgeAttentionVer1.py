...
class HyperEdgeAttention(nn.Module):
    ...
    def __init__(self, channels, edges, heads, bias=False):
        ...
        # Linear Projections
        self.hyperedge = nn.Conv1d(channels, edges, 1, 1, 0, bias=bias)
        self.V = nn.Conv1d(channels, channels, 1, 1, 0, bias=bias)
        self.QK = nn.Conv1d(channels, channels*2, 1, 1, 0, bias=bias)
        self.WE = nn.Conv1d(channels, heads, 1, 1, 0, bias=bias)
    ...
    def forward(self, x):
        ...
        edges = F.rms_norm(weights @ x.transpose(1, 2), [C]).transpose(1, 2)

        # Queries, Keys, Edge weights
        q, k = self.QK(x).view(B, 2, self.heads, self.head_dim, height*width).transpose(-2, -1).unbind(dim=1)
        v = self.V(x).view(B, self.heads, self.head_dim, height*width).transpose(2, 3)
        hq, hk = self.QK(edges).view(B, 2, self.heads, self.head_dim, self.edges).transpose(-2, -1).unbind(dim=1)
        log_WE = self.WE(edges).unsqueeze(-2)

        # Scaled dot-product cross attention implementation
        # Theoretically equivalent to Hypergraph Convolution DinvHW @ (BinvH_T @ v)
        # Though numerical precision causes small values to differ
        hq, hk, q, k, v, log_WE = map(lambda x: x.contiguous(), (hq, hk, q, k, v, log_WE))
        z = F.scaled_dot_product_attention(hq, k, v)
        y = F.scaled_dot_product_attention(q, hk, z, attn_mask=log_WE)
